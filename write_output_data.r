...
loan_out1 <- loans %>%
 ...
 average_balance = bal_avg_loan, tenor = Loan.Tenor, interest_rate =  interest_rate_pred, probability_of_default = pd_w_imputation,
...
write.csv(loan_out1, filename2, row.names = F)
file.show(filename2)

# 2. one row per client, per year
# ----------------------------------------------------------------------------------------------------

client_out1 <- clients %>%
  filter(!is.na(RC.Account.Number | !is.na(Year))) %>%
  distinct(RC.Account.Number, Year, .keep_all = T) %>%
  select(RC.Account.Number, Account.Name, Year, months_outstanding_per_year,
...
filename3 <- paste0('expected_revenue_loan_year_level_', Sys.Date(), '.csv')
write.csv(loan_out1, filename3, row.names = F)
file.show(filename3)
